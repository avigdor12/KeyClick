...
const FINANCE_GUIDE_SECTIONS: Record<string, GuideSection[]> = {
  he: [
    ...
    { heading: 'הכנות והורדת קבצים לטעינה', body: 'הדבר היחיד שיש להכין לפני השימוש באפליקציה הוא להוריד דפי חשבון מהבנק ודפי חשבון מחברת האשראי. האפליקציה תומכת בקבצים מסוג אקסל (xls, xlsx) ו-csv.\n\nיש לציין שלכל דף עו"ש יש להוריד לקובץ אחד, כל קובץ לחשבון אחד. כך גם כרטיסי אשראי, לכל כרטיס אשראי קובץ נפרד.\n\nאיך מורידים דף חשבון מהבנק או חברת אשראי אל המחשב:\n1. פתח דפדפן אינטרנט\n2. התחבר לחשבונך\n3. אם זה עו"ש בחר תקופה\n4. אם זה אשראי בחר כרטיס וחודש חיוב\n5. לחץ על שמירה, בחר פורמט, אקסל או CSV\n6. לחץ להורדה\n7. בחר תיקיה להורדה\n8. סגור חשבון\n9. זכור מיקום הקבצים להמשך טעינה' },
              ...
              return (
                <div className={isFs ? 'guide-doc gd-fs' : 'guide-doc'} style={{ direction: isRTL ? 'rtl' : 'ltr', textAlign: isRTL ? 'right' : 'left', maxWidth: isFs ? 1000 : undefined, marginInline: isFs ? 'auto' : undefined }}>
                  ...
                  {sections.map((s, i) => {
                    ...
                    if (s.sub) return (
                      <div key={i}>{img(s)}<div className="gd-sub">{s.heading}</div><p className="gd-body">{s.body}</p></div>
                    )
                    chNo += 1
                    const n = chNo
                    return (
